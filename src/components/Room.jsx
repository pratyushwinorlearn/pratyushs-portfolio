...
export default function Room({ playerState, isUIOpen, closeUI, ...props }) {
  ...
  React.useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.code === 'KeyE' && isNearRef.current) setIsDoorOpen((prev) => !prev)
    }
  ...
  const isSittingAtDesk = playerState.isSitting && playerState.sitType === 'desk'
    ...
    <>
      <RigidBody type="fixed" colliders="trimesh">
        <group {...props} dispose={null}>
          ...
          <mesh geometry={nodes.Object_52.geometry} position={[3.706, 1.195, -3.981]} rotation={[0, -0.019, 0]} scale={0.01}>
          ...
          <mesh geometry={nodes.Object_74.geometry} position={[3.59, 1.723, -4.116]} rotation={[-Math.PI, -0.081, -Math.PI]} scale={0.01}>
      ...
      <RigidBody type="fixed" colliders={false}>
        <CuboidCollider args={[0.8, 0.5, 1.2]} position={[3.538, 0.5, -4.041]} disabled={isSittingAtDesk} />
      ...
      {/* 🚨 DOOR RIGIDBODY (OBJECT_84 REMOVED FROM HERE) 🚨 */}
      <RigidBody type="kinematicPosition" ref={doorRigidBodyRef} colliders="hull" position={[1.24, 0, -2.518]}>
        <group position={[-1.24, 0, 2.518]}>
          ...
          {/* Object_84 was here causing a massive invisible hull over the desk! Removed successfully. */}
        </group>
      </RigidBody>
      ...
      <group position={[3.571, 1.371, -4.095]} rotation={[0, -0.02, 0]}>
        <Html transform rotation={[0, -Math.PI/2, 0]} position={[1, 0, 0]} scale={0.1}>
          <div id="THE-MISSING-OS" style={{
            position: 'relative', width: '1000px', height: '750px',
